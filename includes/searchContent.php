...
if(isset($_GET['s'])){
	$mainSearch	= sanitize($conn, trim($_GET['s']));

		$countSearch = str_word_count($mainSearch);

if($countSearch>2){
	function first2words($a, $limit=2){
		return preg_replace('/((\w+\W*){'.($limit-1).'}(\w+))(.*)/', '${1}', $a);   
	}
	 $New2Word = first2words($mainSearch);
	
// select individal words
		for($sn=0;$sn<=$countSearch-1;){
			$arr = explode(' ',trim($mainSearch));
				$word	= $sn++;
					$words	= $arr[$word];
		}
// End select individal words
...
<div class="col-sm-12 text-justify" style='margin-bottom:18px;'>
	<div style='margin-bottom:1px;padding:0px;'>You searched for &ldquo;<b><?php echo $mainSearch;?></b>&rdquo;</div>
	<hr/>
	<div style='margin-bottom:10px;padding:20px;background:whitesmoke;'><i>&ldquo;<?php echo $mainSearch;?>&rdquo;</i></div>
</div> 

<?php
$pageArticleQuery = $conn->query("SELECT * FROM `article` WHERE article_status='5' AND (`article_content` LIKE '%$mainSearch%' OR `article_content` LIKE '%$New2Word%' OR `article_title` LIKE '%$New2Word%' OR `article_tags` LIKE '%$New2Word%') ORDER BY `article_id` DESC ");
...
?>
<div class="col-sm-12 text-justify" style='margin-bottom:18px;'>
	<a href='<?php echo $ARTurl;?><?php echo $inludePHP;?>' class='headingLINK'><h3 class='text-left'><?php echo $ARTtitle;?></h3></a>
				
		<a href='<?php echo $ARTurl;?><?php echo $inludePHP;?>' class='subHeadingLINK'>
...
<?php }
	...
	<?php
$pageArticleQuery = $conn->query("SELECT * FROM article WHERE article_status='5' ORDER BY article_date DESC LIMIT 6");
		while($fetchArticleQuery = $pageArticleQuery->fetch_assoc() ){
				$ARTid 			= $fetchArticleQuery['article_id'];
				$ARTuserid 		= $fetchArticleQuery['article_userid'];
				$ARTtitle		 = $fetchArticleQuery['article_title'];
